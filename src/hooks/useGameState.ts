...
// Supabase is the single source of truth — always start fresh, cloud sync will load real data
export const useGameState = () => {
  ...
  const addXp = useCallback((amount: number) => {
    setGameState(prev => {
      ...
      return { ...prev, currentXp: newXp, level: newLevel, maxXp: newMaxXp, rank: newRank };
    });
  }, []);
  ...
  const completeQuest = useCallback((questId: string) => {
    setGameState(prev => {
      ...
      const newState = {
        ...
        currentXp: Math.min(prev.currentXp + quest.xpReward, prev.maxXp * 10),
  ...
  const addHabit = useCallback((habit: Omit<Habit, 'id' | 'streak' | 'completedDays'>) => {
    setGameState(prev => ({
      ...prev,
      habits: [...prev.habits, { ...habit, id: Date.now().toString(), streak: 0, completedDays: Array(30).fill(false) }],
    }));
  }, []);
  ...
  const addQuest = useCallback((quest: Omit<Quest, 'id' | 'completed' | 'failed' | 'createdAt'>) => {
    setGameState(prev => ({
      ...prev,
      quests: [...prev.quests, { ...quest, id: Date.now().toString(), completed: false, failed: false, createdAt: new Date().toISOString() }],
    }));
  }, []);
  ...
  return {
    ...
    deleteHabit, addQuest, deleteQuest, addSystemMessage, isTodayComplete, getCurrentStreak,
  };
};
