...
export const GateEncounter = ({ level, totalQuestsCompleted, currentStreak, onGateDefeated }: GateEncounterProps) => {
  ...
  useEffect(() => {
    ...
    GATES.forEach(gate => {
      const defeated = getProgress(gate) >= gate.challengeTarget;
  ...
  const isDefeated = (gate: typeof GATES[0]) =>
    getProgress(gate) >= gate.challengeTarget;
        ...
        return (
          <motion.div
            ...
            className={cn(
              ...
              defeated ? 'border-green-500/30' : unlocked ? 'border-white/10 hover:border-white/20' : 'border-white/5 opacity-60'
            )}
          >
            {/* Gate header */}
            <button
              ...
              <div className={cn('relative p-4 bg-gradient-to-r to-transparent', gate.headerBg)}>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <h3 className="font-display font-bold text-foreground text-base truncate">{gate.name}</h3>
                    <p className="text-[11px] text-muted-foreground font-jp">{gate.jp}</p>
                  </div>
                ...
                <div className="mt-3 h-1.5 bg-white/5 rounded-full overflow-hidden">
                  <motion.div
                    ...
                    style={!defeated ? { boxShadow: '0 0 8px hsl(var(--primary)/0.5)' } : {}}
                  />
                </div>
              </div>
            </button>
