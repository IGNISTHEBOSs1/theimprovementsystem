        ...
        <div className="flex gap-1.5 overflow-x-auto">
          {CATS.map(c => (
            <button key={c} onClick={() => setCat(c)}
              className={cn('flex-shrink-0 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all',
                ...
                return (
                  <motion.button key={r.id}
                    ...
                    className={cn(
                      ...
                      'border-white/10 bg-white/3 hover:border-accent/30 hover:bg-accent/5 cursor-pointer'
                    )}
                  >
                    {purchased && (
                    ...
                    <p className="font-bold text-xs text-foreground mb-1 line-clamp-1">{r.name}</p>
                    <p className="text-[10px] text-muted-foreground mb-2.5 line-clamp-2 leading-relaxed">{r.description}</p>

                    {locked ? (
                      <p className="text-[9px] text-muted-foreground/50">{unlockLabel(r)}</p>
                    ) : (
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-1">
                          <Coins className="w-3 h-3 text-accent" />
                          <span className={cn('font-display font-bold text-xs', canAfford ? 'text-accent' : 'text-muted-foreground')}>{r.cost}</span>
                        </div>
                        {r.cooldownDays > 1 && !cooldown && (
                          <span className="text-[9px] text-muted-foreground/40">{r.cooldownDays}d cooldown</span>
                        )}
                      </div>
                    )}
                  </motion.button>
