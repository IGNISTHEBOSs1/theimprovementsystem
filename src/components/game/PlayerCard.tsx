...
export const PlayerCard = ({ username, level, rank, currentXp, maxXp, avatarId = 'shadow', streak = 0, questsCompleted = 0, bio }: PlayerCardProps) => {
  ...
  return (
    <motion.div
      ...
      className="relative rounded-2xl overflow-hidden border border-white/8 bg-card"
            ...
            <div className="space-y-1.5">
              ...
              <div className="h-2.5 bg-white/5 rounded-full overflow-hidden border border-white/5">
                <motion.div
                  ...
                  className="h-full rounded-full bg-gradient-to-r from-primary via-primary-glow to-secondary relative overflow-hidden xp-bar"
                />
              </div>
            </div>
          </div>
...
const StatPill = ({ icon, label, value, color, bg }: { icon: React.ReactNode; label: string; value: string; color: string; bg: string }) => (
  <div className={`${bg} rounded-xl p-3 border transition-all duration-200 hover:scale-[1.02]`}>
