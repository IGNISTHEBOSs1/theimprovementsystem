...
export const HabitHeatmap = ({ habit, onToggleDay, index }: HabitHeatmapProps) => {
  ...
  return (
    <motion.div
      ...
      className="glass rounded-2xl p-4 border border-white/10 hover:border-primary/20 transition-colors"
    >
      {/* Header */}
                ...
                <div key={ci} className="relative group">
                  <div
                    ...
                    className={cn(
                      ...
                      tod && done  && 'bg-green-500 text-white cursor-pointer shadow-[0_0_6px_rgba(34,197,94,0.5)]',
                      tod && !done && 'bg-primary/25 border border-primary/50 text-primary ring-1 ring-primary/30 cursor-pointer',
                      past && done  && 'bg-green-600/30 text-green-400/60',
