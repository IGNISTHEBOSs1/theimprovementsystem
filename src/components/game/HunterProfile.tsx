...
import { getXpForLevel } from '@/hooks/useGameState';
...
export const HunterProfile = ({ gameState, achievements, unlockedCount, avatarId = 'shadow', bio, streak }: HunterProfileProps) => {
  ...
  const earnedTitles = TITLES.filter(t => t.condition(gameState));
  const primaryTitle = earnedTitles[earnedTitles.length - 1];

  const statEntries = Object.entries(gameState.stats) as [string, number][];
  const avgStat = Math.round(statEntries.reduce((s, [, v]) => s + v, 0) / statEntries.length);
  ...
  return (
    ...
    >
      {/* Main character card */}
      <div className={cn('relative glass rounded-2xl border overflow-hidden', rankStyle.border, rankStyle.glow && `shadow-lg ${rankStyle.glow}`)}>
        {/* Rank gradient bg */}
        ...
        <div className="relative p-6">
          <div className="flex items-start gap-5">
            ...
            <div className="relative flex-shrink-0">
              <motion.div
                animate={{ boxShadow: [`0 0 20px ${avatar.glow ? 'var(--tw-shadow-color)' : 'transparent'}`, `0 0 40px ${avatar.glow ? 'var(--tw-shadow-color)' : 'transparent'}`, `0 0 20px ${avatar.glow ? 'var(--tw-shadow-color)' : 'transparent'}`] }}
                transition={{ duration: 3, repeat: Infinity }}
                className={cn('w-24 h-24 rounded-2xl overflow-hidden border-2', avatar.border)}
              >
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="mt-3">
                <div className="flex justify-between text-xs mb-1">
                  <span className="text-muted-foreground">Level {gameState.level}</span>
          ...
          <div className="grid grid-cols-3 gap-3 mt-5">
            {[
              ...
              <div key={i} className="bg-white/3 rounded-xl p-3 border border-white/5">
                <div className="flex items-center gap-1.5 mb-1">{stat.icon}<span className="text-[10px] text-muted-foreground">{stat.label}</span></div>
      ...
      <div className="glass rounded-2xl p-5 border border-white/10">
        <h3 className="font-display font-bold text-foreground mb-4 flex items-center gap-2">
          <Sword className="w-4 h-4 text-primary" /> Player Stats
        </h3>
        <div className="grid grid-cols-2 gap-3">
          {statEntries.map(([key, value]) => (
            <div key={key}>
              <div className="flex justify-between text-xs mb-1">
                <span className="font-display font-bold text-muted-foreground">{key}</span>
          ...
          {GATES_CLEARED.map(gate => {
            ...
            return (
              <div key={gate.name} className={cn('flex items-center gap-2 p-2.5 rounded-xl border transition-all',
                ...
                <div className="flex-1 min-w-0">
                  <p className={cn('text-xs font-bold truncate', cleared ? gate.color : 'text-muted-foreground')}>{gate.name}</p>
                  <p className="text-[10px] text-muted-foreground">{gate.rank}-Rank</p>
                </div>
                {cleared
      ...
      <div className="glass rounded-2xl p-5 border border-white/10">
        ...
        <div className="flex flex-wrap gap-2">
          {TITLES.map(t => {
            const earned = t.condition(gameState);
            return (
              <span key={t.title} className={cn('px-3 py-1.5 rounded-xl text-xs font-bold border transition-all',
                earned ? `${t.color} border-white/15 bg-white/5` : 'text-muted-foreground/30 border-white/5 bg-white/2'
              )}>
                {earned ? t.title : '???'}
              </span>
            );
