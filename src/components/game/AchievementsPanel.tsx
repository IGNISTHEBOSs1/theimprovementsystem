...
const CATS = [
  ...
  { key: 'credits', icon: '💰', label: 'Credits', hex: '#63992' },
  ...
  return (
    ...
    >
      ...
      <rect
        ...
        fill={unlocked ? 'hsl(var(--card))' : 'rgba(255,255,255,0.03)'}
        stroke={unlocked ? cfg.border.replace('border-', '').replace('/50','').replace('/40','').replace('/30','') : 'rgba(255,255,255,0.08)'}
        strokeWidth={1.5}
        opacity={hovered ? 1 : 0.9}
        style={{
          filter: unlocked && cfg.glow ? `drop-shadow(0 0 6px ${achievement.rarity === 'epic' ? '#7f77dd' : achievement.rarity === 'legendary' ? '#ba7517' : achievement.rarity === 'mythic' ? '#d4537e' : 'transparent'})` : 'none',
      ...
      {hovered && (
        <g transform="translate(0, -52)">
          <rect x={-60} y={-28} width={120} height={52} rx={6}
            fill="rgba(8,8,18,0.97)" stroke="rgba(255,255,255,0.12)" strokeWidth={0.8} />
          ...
          <text x={0} y={2} textAnchor="middle" fontSize={9} fontWeight="500" fill="rgba(255,255,255,0.9)">
            {achievement.name}
          </text>
          <text x={0} y={14} textAnchor="middle" fontSize={7.5} fill="rgba(255,255,255,0.45)">
