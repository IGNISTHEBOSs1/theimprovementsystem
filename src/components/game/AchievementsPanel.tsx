    ...
    >
      ...
      <motion.div
        ...
        className={`relative w-14 h-14 rounded-2xl border-2 flex items-center justify-center transition-all duration-300 ${
          isUnlocked ? `${cfg.bg} ${cfg.border} shadow-lg ${cfg.glow}` : 'bg-muted/10 border-muted/20 grayscale opacity-40'
        }`}
      >
      ...
      <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-44 px-3 py-2 bg-[#0d0d14]/95 backdrop-blur border border-white/10 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-50 text-center shadow-xl scale-90 group-hover:scale-100">
        ...
        {!isUnlocked && <p className="text-[9px] text-muted-foreground/60 mt-1">🔒 Locked</p>}
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {categories.map((cat, catIdx) => {
              ...
              return (
                ...
                >
                  <button onClick={() => setActiveCategory(isExpanded ? null : cat.key)} className="w-full p-4 text-left">
                    ...
                    {isExpanded && (
                      ...
                      >
                        <div className="p-4 flex flex-wrap gap-2 justify-center">
                          {catAchs.map((ach, i) => (
                            <NodeBadge key={ach.id} achievement={ach} delay={i * 0.04} onClick={() => setSelectedAchievement(ach)} />
                          ))}
                        </div>
                      </motion.div>
                    )}
