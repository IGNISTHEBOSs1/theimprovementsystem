...
const Index = () => {
  ...
  // Show mini-player ONLY when timer is actively running and user leaves quests section
  useEffect(() => {
    try {
      const saved = localStorage.getItem('pomodoro-state');
      if (saved) {
        const parsed = JSON.parse(saved);
        // Only show if timer is RUNNING (not just started or paused)
  ...
  // Today's progress — real counts from gameState
  ...
  const todayQuestProgress = todayQuests.length > 0
    ? Math.round((completedTodayQuests / todayQuests.length) * 100)
    : 0;
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              className="md:hidden fixed inset-x-0 top-16 z-40 mx-3 mt-1"
            >
              <div className="glass-strong rounded-2xl border border-white/12 shadow-2xl shadow-black/40 overflow-hidden">
